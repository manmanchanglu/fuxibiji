...
class Solution {
    ...
    LinkedList<List<Integer>> list=new LinkedList<>();//存放符合要求的结果
    LinkedList<Integer> list1=new LinkedList<>();//存放走过的路径
    public List<List<Integer>> pathSum(TreeNode root, int sum) { 
        int m=0;      
        luJing(root,sum,m);
        return list;
    }
    void luJing(TreeNode root,int sum,int m){
        if(root==null){//递归结束条件
            return;
        }
        m+=root.val;//做选择
        ...
        if(m==sum&&root.left==null&&root.right==null)
            list.add(new LinkedList(list1));//存放路径
        luJing(root.left,sum,m);
        luJing(root.right,sum,m);
        list1.removeLast();//撤销选择
    }
